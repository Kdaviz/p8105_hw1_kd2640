...
####Problem 2.1
The number of rows in `rsample_df` is `r nrow(rsample_df)` and the number of colums is `r ncol(rsample_df)`. 

The mean of `rsample_df` is `r mean(pull(rsample_df, x))`, the median is `r median(pull(rsample, x))`, and the standard deviation is `r sd(pull(rsample_df, x))`.

The number of proportion of cases `r mean(pull(rsample_df, logical_vec))`.
