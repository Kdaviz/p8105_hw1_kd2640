...
columns is `r ncol(rsample_df)`. The mean of `x` in the dataframe `rsample_df` is `r mean(pull(rsample_df,x))`, the median is `r median(pull(rsample_df, x))`, and the standard deviation is `r sd(pull(rsample_df, x))`. The number of proportion cases `r mean(rsample_df$logical_vec)`
